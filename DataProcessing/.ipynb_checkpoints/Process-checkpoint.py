# This will be a conglomerate processing file, containing the contents of EMG_Process
# New_Process and read_data
...
lambda_ = .1033 # Need to check both of these -- I used Vicky's values
pixel_size = 172 # Need to check both of these -- I used the value from this BioCAT site. Units are micrometers.
                 # http://www.bio.aps.anl.gov/techniques/FD-HOWTO.html
...
def Preprocess_eq(eq):
    ...
    # Create a peak label column that will specify L_10, L_11 and L_20, right respectively
    indices = []
    peak_label = []
    fitting_er = []
...
def Preprocess_EMG(EMG_matrix):
    ...
    EMG = pandas.DataFrame(np.arange(0,len(EMG_matrix)))*(1/DAQ_hertz)
    EMG['Trig'] = Trig
    EMG['EMG_amplitude'] = EMG_amplitude
    EMG['EMG_seconds'] = pandas.DataFrame(np.arange(0,len(EMG_amplitude)))*(1/DAQ_hertz)

    EMG = EMG[EMG.Trig > 1] # Cutting to have only EMG in trigger region

    # Reset time to where trigger >1 to start at 0 seconds by substracting universally the first value in the seconds column
    EMG.EMG_seconds = EMG.EMG_seconds - EMG.EMG_seconds.iloc[0]

    # Setting up EMG time base to be used later for interpolating detector data to EMG timebase
    t = EMG.EMG_seconds# np.linspace(0,len(EMG),len(EMG))/DAQ_hertz
    ...
    EMG_output = EMG.rename(index = str, columns = {'seconds':'EMG_seconds'})
    ...
    EMG_output['EMG_amplitude_filter'] = EMG_data

    # Storing peak locations in a binary column

    std_dev = EMG_output['EMG_amplitude_filter'].std()
    thresh = 2.5*std_dev

    p, _ = signal.find_peaks(np.array(1.0*EMG_output['EMG_amplitude_filter']), distance = 800, height = thresh) #, np.arange(30,60,5)) # np.arange(60,80))

    # Create a column that numbers peaks, and is zero else

    peaks = []
    indicator = 0
    for i in range(0,len(EMG_output.EMG_seconds)):
        for j in p:
            if i == j:
                indicator=  1
        peaks.append(indicator)
        indicator = 0
    EMG_output['peaks'] = peaks

    EMG['seconds'] = (1.0/DAQ_hertz)*EMG.index.values.astype(float)
...
def process(Drive_path, day, trial):
    ...
    # Create a seconds column for EMG
    EMG['seconds'] = EMG.index.values.astype(float)*(1.0/DAQ_hertz)
    ...
    ## Edited 10.22: This was made to replace all data with NaNs except for seconds column
    data_cols = ['D10','D11','D20','I10', 'I20I11_I10','I20_I10','tif_num','M3_c',
                'M6_c','A51_c','A59_c','M3_i','M6_i','A59_i','A51_i','Sum']
    ...
    det = det.where(det.ISI > 0).dropna(how = 'all').reset_index(drop = 'True')
    det = det.where(det.ISI < det.ISI.max()).dropna(how = 'all').reset_index(drop = 'True')

    # There has been mixed debate on filling missing data. I have incorporated this
    # in order to keep a fixed time step for DMD
    det[data_cols] = det[data_cols].interpolate(method = 'linear', limit = 2) # fill missing data, but a maximum of two nans at a time
